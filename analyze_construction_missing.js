...
// Construction types - checking variations
...
data.forEach((item, index) => {
    ...
    if (cleanType.startsWith('Bygg')) {
        ...
        // Check if example is missing (item[7] is Swedish example, item[8] is Arabic example)
...
if (allMissing.length > 0) {
    ...
    console.log('\n📝 Sample terms:');
    batch.slice(0, 5).forEach((term, i) => {
        console.log(`   ${i + 1}. ${term.word} (${term.def ? term.def.substring(0, 30) + '...' : 'No def'})`);
    });
}
