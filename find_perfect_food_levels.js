...
function analyze() {
    ...
    // We only care about words that are in our FOOD_KEYWORDS list (or very close)
    for (let mainWord of FOOD_KEYWORDS) {
        ...
        // We need at least 2 pure food words (including the main word itself if it's in the list)
        // Actually, mainWord is in the list by definition of the loop.
        // So we need pureFoodSubWords.length >= 2.
    ...
    console.log("--- Top Perfect Food Levels ---");
    candidates.slice(0, 30).forEach(c => {
        console.log(`\nWORD: ${c.word} (Count: ${c.count})`);
        console.log(`  Targets: ${c.pureFoodSubWords.join(', ')}`);
    });
}
